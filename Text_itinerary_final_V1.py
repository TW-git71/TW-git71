...
with open("Haylese_text_Final_input_10.txt", "w") as file:
    file.write(Prompt)

output_path = r"C:\Evoq\Text_to_Json_Haylise\Text_itinerary_Final\Haylese_text_Final_Output_10.json"
...
class Passage:
    def __init__(self, content, metadata=None):
        ...
        self.metadata = metadata  # Optional metadata
...
def retrieval_function(query, k=10, namespace=namespace):
    ...
    return [Passage(doc.page_content, metadata=doc.metadata) for doc in retrieved_docs]


dspy.settings.configure(lm=gemini,rm= retrieval_function)

# class ItinerarySignature(dspy.Signature):
#     context = dspy.InputField(desc = "All details to generate itinerary")
#     question = dspy.InputField(desc = "User' requirement'")
#     answer = dspy.OutputField(desc="key value pairs")
#     #context = dspy.

class RAG(dspy.Module):
    def __init__(self, num_passages: int = 10):
        super().__init__()
    ...
    def forward(self, question: str):
        ...
        # Pass the search_query directly to the retrieve module
        passages = self.retrieve.forward(query=search_query).passages
        
        # Generate an answer using the retrieved passages and the question
